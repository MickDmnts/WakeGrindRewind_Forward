...
namespace WGR.Gameplay.AI
{
     ...
     * Private variables: These value change throughout the game.
     * 
     * [Must know]
     * 1. Each EnemyEntity instance creates its own Behaviour Tree and BT Handler instances.
     * 2. Values of the NodeData are transfered and changed through the EnemyEntity to the BehaviourTree.
     * 3. On the BossLevel (Level 5) the script calls the GameEventHandler.OnBossLevelEnemyDeath() event.
     * 4. On enemy player death the agent returns to his original position.
     * 5. Agent updates and interactions stop when he's marked as inactive or dead.
     */
    public class EnemyEntity : AIEntity
    {
            ...
            set
            {
                ...
                if (value == true)
                {
                    ...
                    if (GameManager.S.LevelManager.FocusedScene.Equals(GameScenes.Level5))
                    {
                        ...
                        GameManager.S.GameEventHandler.OnBossLevelEnemyDeath();
                    }
                }
        ...
        protected override void CacheComponents()
        {
            ...
            if (!(AIEntityWeapon = GetComponentInChildren<EnemyWeapon>()))
                Utils.MissingComponent("EnemyWeapon", this);

            if (!(EnemyAnimation = GetComponent<EnemyAnimations>()))
        ...
        {
            GameManager.S.AIEntityManager.AddEntityReference(this);
        }
        ...
        void AgentDeath_GlobalNotifiers()
        {
            if (GameManager.S != null)
            {
                GameManager.S.GameEventHandler.OnEnemyDeath();
                float rndShakeStrength = UnityEngine.Random.Range(2f, 7f);
                GameManager.S.GameEventHandler.CameraShakeOnEnemyDeath(0.5f, rndShakeStrength);
            }
